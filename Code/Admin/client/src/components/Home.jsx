...
const Home = ({ isAuthenticated, setTasks, taskTitle, setTaskTitle }) => {
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/v1/task/mytask', { withCredentials: true });
  ...
  const deleteTask = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:5000/api/v1/task/delete/${id}`, { withCredentials: true });
